...
module.exports = function (defaultFuncs, api, ctx) {
  ...
  function sendContent (form, threadID, isSingleUser, messageAndOTID, callback) {
    ...
    defaultFuncs
      ...
      .then(function (resData) {
        if (!resData) {
          return callback(new Error('Send message failed.'));
        }
  ...
  function handleEmoji (msg, form, callback, cb) {
    if (msg.emojiSize != null && msg.emoji == null) {
      return callback(new Error('emoji property is empty'));
    }
    if (msg.emoji) {
  ...
  return function sendMessage (msg, threadID, callback) {
    if (!callback && (utils.getType(threadID) === 'Function' || utils.getType(threadID) === 'AsyncFunction')) {
      return callback(new Error('Pass a threadID as a second argument.'));
    }
    if (!callback) {
      callback = function () {};
    }
