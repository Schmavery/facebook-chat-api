...
function formatDeltaMessage (m) {
  ...
  var mId = mdata.map(u => u.i);
  var mOffset = mdata.map(u => u.o);
  var mLength = mdata.map(u => u.l);
  var mentions = {};
  for (var i = 0; i < mId.length; i++) {
    mentions[mId[i]] = m.delta.body.substring(mOffset[i], mOffset[i] + mLength[i]);
  }
...
function makeDefaults (html, userID, ctx) {
  var reqCounter = 1;
  var fbDtsg = getFrom(html, 'name="fb_dtsg" value="', '"'); // fb_dtsg
  ...
  function mergeWithDefaults (obj) {
    ...
    var newObj = {
      ...
      fb_dtsg: ctx.fb_dtsg ? ctx.fb_dtsg : fbDtsg,
